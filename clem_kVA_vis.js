...
//This function pulls the value from the slider and updates the visualization with the new readings and shows what date you've selected on the website
	slider.oninput = function() {
		...
		chosen_layout.force("collisions").radius(function(kVA) {
			var read = kVA.reading[current]
			if(read == "NA") {return 1}
			return read/50}) 
		chosen_layout.force("link").distance(function(d) {
			var reader = kVA[d.target.index].reading[current]
...
//Forces
    var layout = d3.forceSimulation(kVA_data)
		.force('center', d3.forceCenter(width / 2 , height / 2))
		.force('collisions', d3.forceCollide(function(kVA_data) {
			var read = kVA_data.reading[slider.value]
			if(read == "NA") {return 1}
			return read/50}))
		.force('many', d3.forceManyBody())
		.force('link', d3.forceLink(link).distance(function(d) {
			read = kVA_data[d.target.index].reading[slider.value]
			if (read == "NA") {return 1}
			return read/50})) 
		.on('tick', ticked)
